...
function execLint({ lintResultSrcFile, fix }) {
    ...
    if (!fs.existsSync(path.join(cwd, '/node_modules/stylelint/bin/stylelint.js'))) {
        console.log(red(`\n错误：当前目录没有安装 stylelint，请先执行 ${green('v lint init')} 命令完成初始化\n`));
        return 999; // exitCode
    }

    const lintedFiles = `"${path.join(cwd, '**/*.css')}" "${path.join(cwd, '**/*.less')}" "${path.join(cwd, '**/*.scss')}" "${path.join(cwd, '**/*.sass')}" "${path.join(cwd, '**/*.vue')}"`;
    const order = `${cwd}/node_modules/stylelint/bin/stylelint.js ${lintedFiles} ${fix ? '--fix' : ''} --cache --cache-location ${path.join(path.dirname(lintResultSrcFile), '.stylelintcache')} -f string --custom-formatter ${cwd}/node_modules/stylelint-formatter-html > ${lintResultSrcFile}`;
